...
function bindDashboardPageEvent() {
    ...
    $(document).on('click', '.transferAction', function () {
        ...
        tradingDate = rowObj.data('tradingdate');
        userId = rowObj.data('userid');
        $('#transferPopUp .transferClientName').text(rowObj.data('clientname'));
        $('#transferPopUp .transferTradingDate').text(tradingDate);
        $('#transferPopUp .transfer-amount').val(rowObj.data('transferamount'));
    });
...
function reloadDashboardPartial() {
    ...
    if (validateDateRange(startDate, endDate)) {
        $.ajax({
            url: "/Dashboard/ReloadManagerDashboardPartial",
            ...
            success: function (data) {
                ...
                reloadDashboardLineChart(startDate, endDate, clientUserId);
            },
...
function reloadDashboardLineChart(startDate, endDate, clientUserId) {
    $.ajax({
        url: "/Dashboard/ReloadManagerDashboardLineChart",
        data: { StartDate: startDate, EndDate: endDate, UserId: clientUserId },
        type: "POST",
        success: function (data) {
...
function saveTransferOrder() {
    var transferAmount = $('.transfer-amount').val();
    if(!IsNumber(transferAmount)){
        alert('请输入正确的转账金额');
    }
    else {
        var transferOrderInfo = new Object();
        transferOrderInfo.UserID = userId;
        transferOrderInfo.TrasferDate = tradingDate;
        transferOrderInfo.Amount = transferAmount;
        var transferOrderInfoJsonStr = JSON.stringify(transferOrderInfo);
        $.ajax({
            url: "/Dashboard/SaveTransferOrder",
            data: { TransferOrderJsonStr: transferOrderInfoJsonStr },
            type: "POST",
            success: function (data) {
                if (data == 'success') {
                    $('#transferPopUp').modal("hide");
                    reloadDashboardPartial();
                }
            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
