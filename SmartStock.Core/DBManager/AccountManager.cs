...
namespace SmartStock.Core.DBManager
{
    public class AccountManager
    {
        ...
        public LoginUser AccountSignInValidation(string username, string password)
        {
            ...
            User user = _context.User.FirstOrDefault(x => x.LoginName == username);
        ...
        {
            if (_context.User.Count(x => x.LoginName == userInfo.UserName) > 0)
            {
        ...
        public bool AddUserRole(RegisterInfo userInfo)
        {
            ...
            try
            {
                int userId = _context.User.FirstOrDefault(x => x.LoginName == userInfo.UserName).ID;
